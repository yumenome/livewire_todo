<div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            @if ($todo->completed)
                <input wire:click="toggle({{$todo->id}})" class="mr-1" type="checkbox" checked>
            @else
                <input wire:click="toggle({{$todo->id}})" class="mr-1" type="checkbox">
            @endif

            @if ($edit_id == $todo->id)
                <div class="">
                    <input wire:model="edit_name" type="text" placeholder="Todo.."
...
{{-- del  --}}
    ...
    <div class="mt-3 text-xs text-gray-700">
        @if ($edit_id == $todo->id)
            ...
            class="mt-3 px-4 py-2 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600">Update</button>
            <button wire:click="cancle()"
